...
public class ImageTargetBehaviour extends RenderingBehaviour {

	private Teapot mTeapot;

	public ImageTargetBehaviour(Context context, MyRenderer renderer) {
	...
	public void renderBehaviour(TrackableResult result, float[] modelViewMatrix) {
		if (result.isOfType(ImageTargetResult.getClassType())) {
			...
			int textureIndex = 0;
			if (trackable.getName().equalsIgnoreCase("fh")) {
				textureIndex = 1;
			} else if (trackable.getName().equalsIgnoreCase("android")) {
				textureIndex = 2;
			}
